...
export function createEnemy(
  type: Type, position?: PolarVector): Enemy {
  const pos = position || {
    angle: Math.random() * Math.PI * 2, radius: WORLD_SIZE};
  return {
    age: 0,
    initialPosition: pos,
    position: pos,
...
export function advanceEnemy(
  data: NextPositionData): void {
  data.enemy.age += data.deltaTime;
  data.enemy.position = movePatterns[data.enemy.type](data);
}

const movePatterns: { [type in Type]: (
  data: NextPositionData) => PolarVector } = {
    ...
    Oscillator({enemy, deltaTime}: NextPositionData) {
      const angle = enemy.position.angle +
    enemy.direction * deltaTime * 1;
      const radius = Math.sin(enemy.age * 10) * 50 +
    WORLD_SIZE / 2 - enemy.age * 50;
      const newPos = {
        angle,
        radius,
      };
